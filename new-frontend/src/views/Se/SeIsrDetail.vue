...
// ── Load: circle scope gate via /se/isr/:id, then the rich payload from
// /water-samples/{id}/report (same endpoint the admin Individual Sample
// Report uses — gives us water_sample_details grouped by type).
async function load() {
  loading.value = true
  error.value   = ''
  rawData.value = null
  try {
    await seService.isrShow(route.params.id)              // 404 if outside circle
...
<template>
  <div class="sd">
    <!-- Toolbar (hidden on print via the @media print rule below) -->
    <div class="sd-toolbar isr-toolbar-bar">
      <button class="sd-btn sd-btn-sec" @click="router.back()">← Back</button>
      <div class="spacer"></div>
      <button v-if="report" class="sd-btn sd-btn-sec" @click="printReport">🖨 Print PDF</button>
    </div>

    <div v-if="loading" style="text-align:center;padding:40px;color:#64748b;font-size:13px">
      Loading report...
    </div>
    <div v-else-if="error" style="background:#fef2f2;border:1px solid #fca5a5;color:#991b1b;padding:14px;border-radius:6px;font-size:13px">{{ error }}</div>
