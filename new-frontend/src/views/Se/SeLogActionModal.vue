...
<template>
  ...
  <Teleport to="body">
    <div v-if="modelValue" class="sl-overlay" @click.self="close">
      <div class="sl-modal" role="dialog" aria-modal="true">
        <div class="sl-head">
          <div>
            <div class="sl-title">➕ Log SE Action</div>
            <div class="sl-sub">{{ sampleSlug || sampleId }} — {{ wssName || '—' }}</div>
          </div>
          <button class="sl-close" @click="close" aria-label="Close">✕</button>
        </div>
        <div class="sl-body">
          <div class="sl-grid-2">
          ...
          <div v-if="error" class="sl-err">{{ error }}</div>
        </div>
        <div class="sl-foot">
          <button class="sl-btn sl-btn-sec" @click="close" :disabled="saving">Cancel</button>
          <button class="sl-btn sl-btn-pri" @click="save" :disabled="saving">
...
.sl-title { font-size: 15px; font-weight: 700; color: #0f172a; }
.sl-sub   { font-size: 11.5px; color: #64748b; margin-top: 2px; }
.sl-close {
...
.sl-body { padding: 16px 18px; display: flex; flex-direction: column; gap: 12px; }
...
.sl-btn-sec { background: #fff; color: #334155; border: 1px solid #cbd5e1;
  &:hover:not(:disabled) { background: #f1f5f9; }
}
.sl-btn-pri { background: #1a6bbf; color: #fff;
  &:hover:not(:disabled) { background: #14559c; }
}
@media (max-width: 560px) { .sl-grid-2 { grid-template-columns: 1fr; } }
</style>
