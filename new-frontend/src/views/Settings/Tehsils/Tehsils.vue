...
/* ── Permission gates ──────────────────────────────────────────── */
const canView   = computed(() => userStore.hasPermission('view_tehsils'))
const canAdd    = computed(() => userStore.hasPermission('add_tehsils'))
const canEdit   = computed(() => userStore.hasPermission('edit_tehsils'))
const canDelete = computed(() => userStore.hasPermission('delete_tehsils'))
...
const districtsForForm = computed(() => {
  if (!formDivisionId.value) {
    // No division picked yet — show every district that lives in the picked province
    if (!formProvinceId.value) return districts.value
    return districts.value.filter(d =>
      d.division?.province_id === Number(formProvinceId.value) ||
      divisionsForForm.value.find(dv => dv.id === d.division_id)
    )
  }
...
const filtered = computed(() => {
  let rows = tehsils.value
  const kw = search.value.trim().toLowerCase()
  if (kw) {
    rows = rows.filter(r =>
      (r.name || '').toLowerCase().includes(kw) ||
      (r.district?.name || '').toLowerCase().includes(kw) ||
      (r.district?.division?.name || '').toLowerCase().includes(kw) ||
      (r.district?.division?.province?.name || '').toLowerCase().includes(kw)
    )
  }
  const pf = provinceFilter.value.trim().toLowerCase()
  if (pf) rows = rows.filter(r => (r.district?.division?.province?.name || '').toLowerCase().includes(pf))

  const df = divisionFilter.value.trim().toLowerCase()
  if (df) rows = rows.filter(r => (r.district?.division?.name || '').toLowerCase().includes(df))

  const dt = districtFilter.value.trim().toLowerCase()
  if (dt) rows = rows.filter(r => (r.district?.name || '').toLowerCase().includes(dt))
...
/* Visible page numbers — show a sliding window of 5 so the pagination
   row doesn't blow up when there are 14+ pages (134 tehsils ÷ 10/page). */
...
watch([search, provinceFilter, divisionFilter, districtFilter, nameFilter, pageSize],
      () => { page.value = 1 })
...
async function fetchTehsils() {
  loading.value = true
  try {
    const res = await settingsService.getTehsils()
    tehsils.value = Array.isArray(res?.data) ? res.data : []
  } catch (err) {
    console.error('Fetch tehsils error:', err)
    showToast('❌ Failed to load tehsils: ' + (err?.response?.data?.message || err.message), 'error')
  } finally {
    loading.value = false
  }
}
...
async function fetchSupport() {
  try {
    const [pRes, dRes, dtRes] = await Promise.all([
...
function openEditModal(row) {
  ...
  formProvinceId.value = row.district?.division?.province_id || row.district?.division?.province?.id || ''
  formDivisionId.value = row.district?.division_id || row.district?.division?.id || ''
  formDistrictId.value = row.district_id || row.district?.id || ''
...
async function submitForm() {
  ...
  if (!formName.value.trim()) { formError.value = 'Tehsil name is required.'; return }
  ...
  try {
    if (editingId.value) {
      await settingsService.updateTehsil(editingId.value, payload)
      showToast('✅ Tehsil updated successfully.')
    } else {
      await settingsService.createTehsil(payload)
      showToast('✅ Tehsil added successfully.')
    }
    showModal.value = false
    await fetchTehsils()
  } catch (err) {
    console.error('Save tehsil error:', err)
    const e = err?.response?.data
    formError.value = e?.errors
      ? Object.values(e.errors).flat().join('\n')
      : (e?.message || err.message || 'Failed to save tehsil.')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await settingsService.deleteTehsil(deleteTarget.value.id)
    showToast(`🗑 Deleted tehsil "${deleteTarget.value.name}".`)
    deleteTarget.value = null
    await fetchTehsils()
  } catch (err) {
    console.error('Delete tehsil error:', err)
    showToast('❌ ' + (err?.response?.data?.message || 'Failed to delete tehsil'), 'error')
  } finally {
...
function exportCsv() {
  const rows = filtered.value.map(r => ({
    'Province':   r.district?.division?.province?.name || '',
    'Division':   r.district?.division?.name || '',
  ...
  downloadCSV(arrayToCSV(rows), `tehsils_${new Date().toISOString().slice(0, 10)}.csv`)
}

onMounted(async () => {
  await Promise.all([fetchTehsils(), fetchSupport()])
})
</script>
...
<template>
  <div class="settings-page">
    ...
    <div class="breadcrumbs">
      ...
      <span>Dashboard</span>
      <span class="bc-sep">›</span>
      <span class="active">Tehsils</span>
    </div>
    ...
    <section class="data-card" v-if="canView">
      <header class="card-header">
        <h1>Tehsils</h1>
        <button v-if="canAdd" class="btn-primary" @click="openAddModal">+ Add Tehsil</button>
      </header>
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr class="filter-row">
              <td><input class="col-filter" v-model="provinceFilter" placeholder="District" /></td>
              ...
              <td><input class="col-filter" v-model="nameFilter" placeholder="Tehsil" /></td>
              <td></td>
              <td></td>
            </tr>
            ...
            <tr v-else-if="paged.length === 0">
              <td colspan="6" class="empty-row">
                {{ (search || provinceFilter || divisionFilter || districtFilter || nameFilter)
                    ? 'No tehsils match your search.'
                    : 'No tehsils yet. Click "Add Tehsil" to create one.' }}
              </td>
            </tr>

            <!-- Data rows -->
            <tr v-else v-for="row in paged" :key="row.id" class="data-row">
              <td>{{ row.district?.division?.province?.name || '—' }}</td>
    ...
    <div v-else class="forbidden-card">🚫 You don't have permission to view Tehsils.</div>
        ...
        <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
          <div class="modal">
            <header class="modal-header">
              <h2>{{ editingId ? 'Edit Tehsil' : 'Add Tehsil' }}</h2>
            ...
            <div class="modal-body">
              ...
              <label class="field-label" style="margin-top:14px;">Tehsil Name <span class="required">*</span></label>
              <input class="field-input" v-model="formName"
                     placeholder="e.g. Peshawar city" :disabled="saving" maxlength="255" />
            ...
            <footer class="modal-footer">
              ...
              <button class="btn-primary" @click="submitForm" :disabled="saving">
                {{ saving ? 'Saving…' : (editingId ? 'Update Tehsil' : 'Add Tehsil') }}
              </button>
            </footer>
          </div>
        </div>
        ...
        <div v-if="deleteTarget" class="modal-overlay" @click.self="cancelDelete">
          <div class="modal modal-sm">
            ...
            <div class="modal-body">
              ...
              <p class="muted small">This action cannot be undone. Tehsils with attached union councils cannot be deleted.</p>
            </div>
            <footer class="modal-footer">
