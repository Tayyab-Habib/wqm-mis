...
/* ── Permission gates ──────────────────────────────────────────── */
const canView   = computed(() => userStore.hasPermission('view_units'))
const canAdd    = computed(() => userStore.hasPermission('add_units'))
const canDelete = computed(() => userStore.hasPermission('delete_units'))

/* ── Type label mapping ────────────────────────────────────────────
   Backend stores `material` (consumables) and `asset` (durable). The
   screenshot shows them as "Stock" and "Inventory" respectively — keep
   that mapping in ONE place so the UI labels stay consistent everywhere
   we render the type. */
const TYPE_LABEL = {
  material: 'Stock',
  asset:    'Inventory',
...
function typeBadgeClass(t) {
  return t === 'material' ? 'type-stock' : (t === 'asset' ? 'type-inventory' : 'type-other')
}
...
const filtered = computed(() => {
  ...
  if (kw) {
    rows = rows.filter(r =>
      ...
      typeLabel(r.type).toLowerCase().includes(kw)
    )
  }
  ...
  if (tf) rows = rows.filter(r => typeLabel(r.type).toLowerCase().includes(tf))
...
watch([search, nameFilter, typeFilter, pageSize], () => { page.value = 1 })

/* ── API actions ──────────────────────────────────────────────── */
async function fetchUnits() {
  loading.value = true
  try {
    const res = await settingsService.getUnits()
    units.value = Array.isArray(res?.data) ? res.data : []
  } catch (err) {
    console.error('Fetch units error:', err)
    showToast('❌ Failed to load units: ' + (err?.response?.data?.message || err.message), 'error')
  } finally {
    loading.value = false
  }
}
...
async function submitForm() {
  formError.value = ''
  if (!formName.value.trim()) { formError.value = 'Unit name is required.'; return }
  if (!formType.value)        { formError.value = 'Type is required.'; return }
  ...
  saving.value = true
  try {
    await settingsService.createUnit(payload)
    ...
    formError.value = e?.errors
      ...
      : (e?.message || err.message || 'Failed to save unit.')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await settingsService.deleteUnit(deleteTarget.value.id)
    showToast(`🗑 Deleted unit "${deleteTarget.value.name}".`)
...
function exportCsv() {
  ...
  downloadCSV(arrayToCSV(rows), `units_${new Date().toISOString().slice(0, 10)}.csv`)
}
  ...
  <div class="settings-page">
    ...
    <section class="data-card" v-if="canView">
      <header class="card-header">
        <h1>Units</h1>
        <button v-if="canAdd" class="btn-primary" @click="openAddModal">+ Add Units</button>
      </header>
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr class="filter-row">
              <td><input class="col-filter" v-model="nameFilter" placeholder="Units Name" /></td>
              <td><input class="col-filter" v-model="typeFilter" placeholder="Units Type" /></td>
              <td></td>
              <td></td>
            </tr>
            ...
            <tr v-else-if="paged.length === 0">
              <td colspan="4" class="empty-row">
                {{ (search || nameFilter || typeFilter)
                    ? 'No units match your search.'
                    : 'No units yet. Click "Add Units" to create one.' }}
              </td>
            </tr>
            ...
            <tr v-else v-for="row in paged" :key="row.id" class="data-row">
              ...
              <td><span class="type-pill" :class="typeBadgeClass(row.type)">{{ typeLabel(row.type) }}</span></td>
    ...
    <div v-else class="forbidden-card">🚫 You don't have permission to view Units.</div>
          ...
          <div class="modal">
            ...
            <div class="modal-body">
              ...
              <input class="field-input" v-model="formName"
                     placeholder="e.g. Kg, Bottle, Pack" :disabled="saving" maxlength="255"
                     @keyup.enter="submitForm" />

              <label class="field-label" style="margin-top:14px;">Type <span class="required">*</span></label>
              <select class="field-input" v-model="formType" :disabled="saving">
                <option v-for="opt in TYPE_OPTIONS" :key="opt.value" :value="opt.value">
                  {{ opt.label }}
                </option>
              </select>
              <p class="muted small" style="margin-top:6px;">
                <strong>Stock</strong> = consumables (mg, ml, L); <strong>Inventory</strong> = durable items (Bag, Pack, Bottle).
              </p>
        ...
        <div v-if="deleteTarget" class="modal-overlay" @click.self="cancelDelete">
          <div class="modal modal-sm">
            ...
            <div class="modal-body">
              ...
              <p class="muted small">This action cannot be undone. Units referenced by tests or materials cannot be deleted.</p>
            </div>
            <footer class="modal-footer">
...
.type-stock     { background: #dbeafe; color: #1e3a8a; border-color: #93c5fd; }
.type-inventory { background: #ede9fe; color: #5b21b6; border-color: #c4b5fd; }
...
.sk-pill { width: 70px; height: 18px; border-radius: 9px; }
