...
/* ── Permission gates ──────────────────────────────────────────── */
const canView   = computed(() => userStore.hasPermission('view_designations'))
const canAdd    = computed(() => userStore.hasPermission('add_designations'))
const canEdit   = computed(() => userStore.hasPermission('edit_designations'))
const canDelete = computed(() => userStore.hasPermission('delete_designations'))
...
async function fetchDesignations() {
  loading.value = true
  try {
    const res = await settingsService.getDesignations()
    designations.value = Array.isArray(res?.data) ? res.data : []
  } catch (err) {
    console.error('Fetch designations error:', err)
    showToast('❌ Failed to load designations: ' + (err?.response?.data?.message || err.message), 'error')
  } finally {
    loading.value = false
  }
}
...
async function submitForm() {
  ...
  if (!formName.value.trim()) {
    formError.value = 'Designation name is required.'
    return
  }
  ...
  try {
    if (editingId.value) {
      await settingsService.updateDesignation(editingId.value, payload)
      showToast('✅ Designation updated successfully.')
    } else {
      await settingsService.createDesignation(payload)
    ...
    formError.value = e?.errors
      ...
      : (e?.message || err.message || 'Failed to save designation.')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await settingsService.deleteDesignation(deleteTarget.value.id)
    showToast(`🗑 Deleted designation "${deleteTarget.value.name}".`)
...
function exportCsv() {
  ...
  downloadCSV(arrayToCSV(rows), `designations_${new Date().toISOString().slice(0, 10)}.csv`)
}
...
<template>
  <div class="settings-page">
    ...
    <div class="breadcrumbs">
      ...
      <span>Dashboard</span>
      <span class="bc-sep">›</span>
      <span class="active">Designations</span>
    </div>
    ...
    <section class="data-card" v-if="canView">
      <header class="card-header">
        <h1>Designations</h1>
        <button v-if="canAdd" class="btn-primary" @click="openAddModal">+ Add Designation</button>
      </header>
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr class="filter-row">
              <td><input class="col-filter" v-model="nameFilter" placeholder="Designations Name" /></td>
              <td></td>
              <td></td>
            </tr>
            ...
            <template v-if="loading">
              <tr v-for="i in 8" :key="`sk-${i}`" class="sk-row">
                <td><span class="sk sk-text" style="width:40%"></span></td>
    ...
    <div v-else class="forbidden-card">🚫 You don't have permission to view Designations.</div>
          ...
          <div class="modal">
            ...
            <footer class="modal-footer">
              ...
              <button class="btn-primary" @click="submitForm" :disabled="saving">
                {{ saving ? 'Saving…' : (editingId ? 'Update Designation' : 'Add Designation') }}
              </button>
            </footer>
          </div>
        </div>
        ...
        <div v-if="deleteTarget" class="modal-overlay" @click.self="cancelDelete">
          <div class="modal modal-sm">
            ...
            <div class="modal-body">
              ...
              <p class="muted small">This action cannot be undone. Designations referenced by user profiles cannot be deleted.</p>
            </div>
            <footer class="modal-footer">
