...
/* ── Permission gates ──────────────────────────────────────────── */
const canView   = computed(() => userStore.hasPermission('view_abbreviations'))
const canAdd    = computed(() => userStore.hasPermission('add_abbreviations'))
const canEdit   = computed(() => userStore.hasPermission('edit_abbreviations'))
const canDelete = computed(() => userStore.hasPermission('delete_abbreviations'))

/* ── State ─────────────────────────────────────────────────────── */
const loading        = ref(false)
const saving         = ref(false)
const deleting       = ref(false)
const abbreviations  = ref([])
...
async function fetchAbbreviations() {
  loading.value = true
  try {
    const res = await settingsService.getAbbreviations()
    abbreviations.value = Array.isArray(res?.data) ? res.data : []
  } catch (err) {
    console.error('Fetch abbreviations error:', err)
    showToast('❌ Failed to load abbreviations: ' + (err?.response?.data?.message || err.message), 'error')
  } finally {
    loading.value = false
  }
}
...
async function submitForm() {
  formError.value = ''
  if (!formName.value.trim())   { formError.value = 'Abbreviation is required.'; return }
  if (!formDetail.value.trim()) { formError.value = 'Detail (full form) is required.'; return }
  ...
  try {
    if (editingId.value) {
      await settingsService.updateAbbreviation(editingId.value, payload)
      showToast('✅ Abbreviation updated successfully.')
    } else {
      await settingsService.createAbbreviation(payload)
      showToast('✅ Abbreviation added successfully.')
    }
    showModal.value = false
    await fetchAbbreviations()
  } catch (err) {
    console.error('Save abbreviation error:', err)
    const e = err?.response?.data
    formError.value = e?.errors
      ? Object.values(e.errors).flat().join('\n')
      : (e?.message || err.message || 'Failed to save abbreviation.')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await settingsService.deleteAbbreviation(deleteTarget.value.id)
    showToast(`🗑 Deleted abbreviation "${deleteTarget.value.name}".`)
    deleteTarget.value = null
    await fetchAbbreviations()
  } catch (err) {
    console.error('Delete abbreviation error:', err)
    showToast('❌ ' + (err?.response?.data?.message || 'Failed to delete abbreviation'), 'error')
  } finally {
...
function exportCsv() {
  ...
  downloadCSV(arrayToCSV(rows), `abbreviations_${new Date().toISOString().slice(0, 10)}.csv`)
}

onMounted(fetchAbbreviations)
</script>
...
<template>
  <div class="settings-page">
    ...
    <div class="breadcrumbs">
      ...
      <span>Dashboard</span>
      <span class="bc-sep">›</span>
      <span class="active">Abbreviation</span>
    </div>
    ...
    <section class="data-card" v-if="canView">
      <header class="card-header">
        <h1>Abbreviations</h1>
        <button v-if="canAdd" class="btn-primary" @click="openAddModal">+ Create Abbreviation</button>
      </header>
      ...
      <div class="table-wrap">
        <table class="data-table">
          <thead>
            <tr>
              <th style="width:240px;">Abbreviation</th>
              <th>Detail</th>
          ...
          <tbody>
            ...
            <tr class="filter-row">
              <td><input class="col-filter" v-model="nameFilter" placeholder="Name" /></td>
              <td></td>
              <td></td>
              <td></td>
            </tr>
            ...
            <tr v-else-if="paged.length === 0">
              <td colspan="4" class="empty-row">
                {{ (search || nameFilter)
                    ...
                    : 'No abbreviations yet. Click "Create Abbreviation" to add one.' }}
              </td>
            </tr>

            <!-- Data rows -->
            <tr v-else v-for="row in paged" :key="row.id" class="data-row">
              <td><span class="abbr-pill">{{ row.name }}</span></td>
              <td>{{ row.detail }}</td>
    ...
    <div v-else class="forbidden-card">🚫 You don't have permission to view Abbreviations.</div>
        ...
        <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
          <div class="modal">
            <header class="modal-header">
              <h2>{{ editingId ? 'Edit Abbreviation' : 'Create Abbreviation' }}</h2>
              <button class="modal-close" @click="closeModal" :disabled="saving">✕</button>
            </header>

            <div class="modal-body">
              <label class="field-label">Abbreviation <span class="required">*</span></label>
              <input class="field-input mono" v-model="formName"
                     placeholder="e.g. NGVS" :disabled="saving" maxlength="255"
                     style="text-transform: uppercase" />

              <label class="field-label" style="margin-top:14px;">
                Detail (Full Form) <span class="required">*</span>
              </label>
              <textarea class="field-input" v-model="formDetail" rows="3"
                        placeholder="e.g. No Guideline Value Set"
                        :disabled="saving" maxlength="65535"></textarea>
            ...
            <footer class="modal-footer">
              ...
              <button class="btn-primary" @click="submitForm" :disabled="saving">
                {{ saving ? 'Saving…' : (editingId ? 'Update Abbreviation' : 'Create Abbreviation') }}
              </button>
            </footer>
          </div>
        </div>
...
/* Abbreviation pill — distinguishes the code visually from the long detail */
.abbr-pill {
...
.field-input.mono { font-family: 'SF Mono', Menlo, Consolas, monospace; letter-spacing: 0.5px; }
