...
/* ── Permission gates ──────────────────────────────────────────── */
const canView   = computed(() => userStore.hasPermission('view_districts'))
const canAdd    = computed(() => userStore.hasPermission('add_districts'))
const canEdit   = computed(() => userStore.hasPermission('edit_districts'))
const canDelete = computed(() => userStore.hasPermission('delete_districts'))
...
/* ── Cascading dropdowns: divisions filtered by selected province ── */
...
watch(formProvinceId, () => {
  // Reset the division choice if it no longer belongs to the selected province
...
const filtered = computed(() => {
  let rows = districts.value
  const kw = search.value.trim().toLowerCase()
  if (kw) {
    rows = rows.filter(r =>
      (r.name || '').toLowerCase().includes(kw) ||
      (r.division?.name || '').toLowerCase().includes(kw) ||
      (r.division?.province?.name || '').toLowerCase().includes(kw)
    )
  }
  const pf = provinceFilter.value.trim().toLowerCase()
  if (pf) rows = rows.filter(r => (r.division?.province?.name || '').toLowerCase().includes(pf))

  const df = divisionFilter.value.trim().toLowerCase()
  if (df) rows = rows.filter(r => (r.division?.name || '').toLowerCase().includes(df))
...
watch([search, provinceFilter, divisionFilter, nameFilter, pageSize], () => { page.value = 1 })

/* ── API actions ──────────────────────────────────────────────── */
async function fetchDistricts() {
  loading.value = true
  try {
    const res = await settingsService.getDistricts()
    districts.value = Array.isArray(res?.data) ? res.data : []
  } catch (err) {
    console.error('Fetch districts error:', err)
    showToast('❌ Failed to load districts: ' + (err?.response?.data?.message || err.message), 'error')
  } finally {
    loading.value = false
  }
}
...
async function fetchSupport() {
  // Provinces + divisions feed the dropdowns inside the Add/Edit modal.
  try {
...
function openEditModal(row) {
  ...
  formProvinceId.value = row.division?.province_id || row.division?.province?.id || ''
  formDivisionId.value = row.division_id || row.division?.id || ''
...
async function submitForm() {
  ...
  if (!formName.value.trim()) { formError.value = 'District name is required.'; return }
  ...
  try {
    if (editingId.value) {
      await settingsService.updateDistrict(editingId.value, payload)
      showToast('✅ District updated successfully.')
    } else {
      await settingsService.createDistrict(payload)
    ...
    formError.value = e?.errors
      ...
      : (e?.message || err.message || 'Failed to save district.')
  } finally {
    saving.value = false
  }
}
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    await settingsService.deleteDistrict(deleteTarget.value.id)
    showToast(`🗑 Deleted district "${deleteTarget.value.name}".`)
    deleteTarget.value = null
    await fetchDistricts()
  } catch (err) {
    console.error('Delete district error:', err)
    showToast('❌ ' + (err?.response?.data?.message || 'Failed to delete district'), 'error')
  } finally {
...
function exportCsv() {
  const rows = filtered.value.map(r => ({
    'Province':      r.division?.province?.name || '',
  ...
  downloadCSV(arrayToCSV(rows), `districts_${new Date().toISOString().slice(0, 10)}.csv`)
}

onMounted(async () => {
  await Promise.all([fetchDistricts(), fetchSupport()])
})
</script>
  ...
  <div class="settings-page">
    ...
    <section class="data-card" v-if="canView">
      <header class="card-header">
        <h1>Districts</h1>
        <button v-if="canAdd" class="btn-primary" @click="openAddModal">+ Add District</button>
      </header>
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr class="filter-row">
              ...
              <td><input class="col-filter" v-model="nameFilter" placeholder="District" /></td>
              <td></td>
              <td></td>
            </tr>
            ...
            <tr v-else-if="paged.length === 0">
              <td colspan="5" class="empty-row">
                {{ (search || provinceFilter || divisionFilter || nameFilter)
            ...
            <tr v-else v-for="row in paged" :key="row.id" class="data-row">
              <td>{{ row.division?.province?.name || '—' }}</td>
              ...
              <td class="text-right">
                ...
                <button class="btn-icon btn-view" @click="openViewModal(row)" title="View">👁</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-else class="forbidden-card">🚫 You don't have permission to view Districts.</div>
        ...
        <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
          <div class="modal">
            ...
            <div class="modal-body">
              ...
              <label class="field-label" style="margin-top:14px;">District Name <span class="required">*</span></label>
            ...
            <footer class="modal-footer">
              ...
              <button class="btn-primary" @click="submitForm" :disabled="saving">
                {{ saving ? 'Saving…' : (editingId ? 'Update District' : 'Add District') }}
              </button>
            </footer>
          </div>
        </div>
        ...
        <div v-if="viewTarget" class="modal-overlay" @click.self="viewTarget = null">
          <div class="modal">
            ...
            <div class="modal-body">
              <dl class="view-list">
                <dt>Province</dt>
                <dd>{{ viewTarget.division?.province?.name || '—' }}</dd>
                <dt>Division</dt>
                <dd>{{ viewTarget.division?.name || '—' }}</dd>
                <dt>District</dt>
                <dd class="fw-700">{{ viewTarget.name }}</dd>
                <dt>Latitude / Longitude</dt>
                <dd>{{ viewTarget.latitude || '—' }} / {{ viewTarget.longitude || '—' }}</dd>
                <dt>Created At</dt>
                <dd>{{ formatDate(viewTarget.created_at) }}</dd>
              </dl>
            </div>
            <footer class="modal-footer">
              <button class="btn-secondary" @click="viewTarget = null">Close</button>
            </footer>
          </div>
        </div>
      </Transition>
        ...
        <div v-if="deleteTarget" class="modal-overlay" @click.self="cancelDelete">
          <div class="modal modal-sm">
            ...
            <div class="modal-body">
              ...
              <p class="muted small">This action cannot be undone. Districts with attached tehsils cannot be deleted.</p>
            </div>
            <footer class="modal-footer">
...
.btn-view { background: #dbeafe; color: #1e3a8a; border-color: #93c5fd; margin-left: 4px; }
...
.view-list { display: grid; grid-template-columns: 140px 1fr; gap: 8px 14px; margin: 0; font-size: 13px; }
.view-list dt { color: #64748b; font-weight: 600; }
.view-list dd { margin: 0; color: #0f172a; }
